...
function here () {
	...
	if (options.indexOf('cache') !== -1 
	&& hereCache[fileName]
	&& var cached = hereCache[fileName][pos+'X'+len]) 
		return cached;
	...
	if (options.indexOf('cache') !== -1) {
		hereCache[fileName] = hereCache[fileName] || {};
		cache[fileName][pos+'X'+len] = body;
	}
	return body;
}
...
function section (name) {
	...
	if (!sectionCache[filename]) {
		...
		sectionCache[filename] = data;
	}

	return name ? sectionCache[filename][name] : cache[filename];
}
